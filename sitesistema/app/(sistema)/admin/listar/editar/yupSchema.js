...
export const Yupschema = yup.object({
  tipo: yup
    .number()
    .typeError("O tipo deve ser um número")
  ...
  cpf: yup.string().when("tipo", {
    is: (tipo) => tipo === 3 || tipo === "3", // Validar CPF se for Paciente
    then: (schema) =>
      schema
        .matches(/^\d{11}$/, "CPF deve conter exatamente 11 dígitos.")
        .required("O CPF é obrigatório para cadastro de Pacientes"),
    otherwise: (schema) => schema.strip(), // remove do schema para Admin/Secretaria
  }),
