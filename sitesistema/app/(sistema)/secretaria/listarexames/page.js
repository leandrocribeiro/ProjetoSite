...
import { ListarExames } from "./apiListarExames";
import { DeletarExames } from "./apiDeletarExames";
...
export default function Listar() {
  ...
  async function excluir(id) {
    try {
      const resultado = await DeletarExames(id);
                ...
                {dados.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-center">
                      <Link
                        href={`/secretaria/listarexames/editarexames?id=${item.id}`}
                      >
